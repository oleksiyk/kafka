    ...
    return self._waitMetadata().then(function () {
        ...
        .then(function (topics) {
            ...
            result = _mapTopics(topics).map(function (r) {
                if (r.messageSet && r.messageSet.length === 1 && r.messageSet[0].message.attributes.codec !== 0) {
                    if (self.options.asyncCompression) {
                        compressionPromises.push(self._decompressMessageSet(r.messageSet[0].message)
                            .then(function (messageSet) {
                                r.messageSet = messageSet;
                            })
                            .catch(function (err) {
                                self.error('Failed to decompress message at', r.topic + ':' + r.partition + '@' + r.messageSet[0].offset, err);
                            }));
                    } else {
                        try {
                            r.messageSet = self._decompressMessageSet(r.messageSet[0].message);
                        } catch (err) {
                            self.error('Failed to decompress message at', r.topic + ':' + r.partition + '@' + r.messageSet[0].offset, err);
                        }
                    }
