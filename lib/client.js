...
Client.prototype._resetGroupCoordinator = function(groupId) {
    var self = this;
    if(self.groupCoordinators[groupId] && self.groupCoordinators[groupId].isResolved()){
...
Client.prototype.joinConsumerGroupRequest = function(groupId, memberId, sessionTimeout, strategies) {
    ...
    return self._findGroupCoordinator(groupId).then(function (connection) {
        ...
        return connection.send(buffer).then(function (responseBuffer) {
            ...
            if(result.error){
                if(result.error.code === 'NotCoordinatorForGroup'){
                    self._resetGroupCoordinator(groupId);
                }
                throw result.error;
            }
            return result;
