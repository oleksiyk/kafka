...
Client.prototype.init = function () {
    ...
    return p.then(function () {
        self.initialBrokers = self.options.connectionString.split(',').map(function (hostStr) {
            ...
            return self._createConnection(parsed.hostname, parseInt(parsed.port));
        });
    ...
    self._updateMetadata_running = (function _try() {
        return self.metadataRequest().then(function (response) {
            ...
            _.each(response.broker, function (broker) {
                ...
                self.brokerConnections[broker.nodeId] = connection || self._createConnection(broker.host, broker.port);
            });
...
Client.prototype._findGroupCoordinator = function (groupId) {
    ...
    .then(function (host) {
        return self._createConnection(host.coordinatorHost, host.coordinatorPort);
    })
    .catch(function (err) {
        if (err.length === 1) {
            throw err[0];
