...
Client.prototype.offsetFetchRequest = function(groupId, requests) {
    var self = this;
...
Client.prototype._findGroupCoordinator = function(groupId) {
    ...
    if(self.groupCoordinators[groupId] && !Promise.isRejected(self.groupCoordinators[groupId])){
...
Client.prototype.joinConsumerGroupRequest = function(groupId, memberId, sessionTimeout, protocols) {
    var self = this;
    ...
    return self._findGroupCoordinator(groupId).then(function (connection) {
        ...
        return connection.send(buffer).then(function (responseBuffer) {
            return protocol.read(responseBuffer).JoinGroupResponse().result;
        });
    });
};
