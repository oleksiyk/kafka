    ...
    function _try (_data, attempt) {
        ...
        return self._prepareProduceRequest(_data).then(function (requests) {
            return self.client.produceRequest(requests).then(function (response) {
                ...
                return Promise.map(response, function (rt) {
                    return Promise.map(rt.partitions, function (p) {
                        if(p.error){
                            if(/UnknownTopicOrPartition|NotLeaderForPartition|LeaderNotAvailable/.test(p.error.code) && attempt < 3){
                                toRetry = toRetry.concat(_.filter(_data, {topic: rt.topicName, partition: p.partition}));
                            } else {
                                result.push({
                                    topic: rt.topicName,
