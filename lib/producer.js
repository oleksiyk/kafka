    ...
    return Promise.map(data, function (d) {
        return Promise.try(function () {
            if (typeof d.partition !== 'number' || d.partition < 0) {
                return self.client.getTopicPartitions(self.client.topic).then(function (partitions) {
                    return self.partitioner.partition(self.client.topic, partitions, d.message);
                })
        ...
        .then(function () {
            return self.client.findLeader(self.client.topic, d.partition).then(function (leader) {
                d.leader = leader;
                d.topic = self.client.topic;
            });
        });
                ...
                return Promise.map(response, function (p) {
                    if (p.error) {
                        if ((/UnknownTopicOrPartition|NotLeaderForPartition|LeaderNotAvailable/.test(p.error.code)
                            ...
                            self.client.debug('Received', p.error, 'for', self.client.topic + ':' + p.partition);
                            toRetry = toRetry.concat(_.filter(_data, { partition: p.partition }));
                        } else {
                            result.push(p);
                        }
                    } else {
