...
function GroupConsumer (options){
    this.options = _.partialRight(_.merge, _.defaults)(options || {}, {
        ...
        startFrom: null // { offset, time }, start from specified offset or time instead of committed value
    });
    ...
    return self.client.updateMetadata().then(function () {
        return self.fetchOffset(offsetRequests).then(function (result) {
            return Promise.map(result, function (p) {
                if(self.options.startFrom){
                    return self.subscribe(p.topic, p.partition, self.options.startFrom);
                }
        ...
        .tap(function () {
            if(self.options.startFrom){ // use this option only during initial startup
                self.options.startFrom = null;
            }
        });
    });
};
