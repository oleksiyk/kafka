...
GroupConsumer.prototype._joinGroup = function() {
    ...
    .then(function (response) {
        if(self.memberId){
            Kafka.log('Joined group', self.options.groupId, 'generationId', response.generationId, 'as', response.memberId);
            ...
            return self.client.updateMetadata().then(function () {
                ...
                r = _(r).groupBy(0).map(function (val, key) {
                    if(!self.client.topicMetadata[key]){
                        Kafka.error('Sync group: unknown topic:', key);
                    }
                    return {
                        topic: key,
                        members: _.map(val, 1),
...
GroupConsumer.prototype._heartbeat = function() {
    ...
    .catch(function (err) {
        if(self._finished){ return }
        Kafka.error('Sending heartbeat failed: ', err);
        return self._fullRejoin().catch(function (err) {
            Kafka.error(err);
        });
    })
    .tap(function () {
...
GroupConsumer.prototype._updateSubscriptions = function(partitionAssignment) {
    ...
    if(_.isEmpty(partitionAssignment)){
        return Kafka.warning('No partition assignment received');
    }
    ...
    return self.client.updateMetadata().then(function () {
        return self.fetchOffset(offsetRequests).then(function (result) {
            return Promise.map(result, function (p) {
                ...
                .catch(function (err) {
                    Kafka.error('Failed to subscribe to', p.topic + ':' + p.partition, err);
                });
            });
        });
    });
