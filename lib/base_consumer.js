...
BaseConsumer.prototype._partitionError = function (err, topic, partition) {
    ...
    /* istanbul ignore next */
        ...
        self.client.debug('Received', err.code, 'error for', topic + ':' + partition, 'from', self.client.leaderServer(s.leader));
        return self._updateSubscription(topic, partition);
    /* istanbul ignore next */
    } else if (err instanceof errors.NoKafkaConnectionError) {
        self.client.debug('Received connection error for', topic + ':' + partition, 'from', self.client.leaderServer(s.leader));
        return self._updateSubscription(topic, partition);
    }
